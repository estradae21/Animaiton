...
/**
 * Created by ernestoestrada on 3/1/18.
 */
...
(context: Context) : View(context) {
    ...
    fun setStyle(s: Int) {
        ...
        colorFlag = s == COLOR_ACC || s == MOVE_RECOLOR || s == MOVE_ROTATE_RECOLOR
                || s == BOUNCE_RECOLOR || s == ROTATE_COLOR
    ...
    fun wander() {
        ...
        when (style) {
            ...
            ROTATE_COLOR -> {
                ...
                val recolorguy = getObjectAnimator(500, "currColor", 0.0f, 1.0f)
                val atOnce = AnimatorSet()
                atOnce.play(spinguy).with(recolorguy)
                atOnce.start()
            }
        /*
    ...
    companion object {  //static stuff
        ...
        const val BOUNCE_RECOLOR = 4
        const val ROTATE_COLOR = 5
    }
}